...
class PrintCFG(AstFullTraverser):
    ...
    def do_FunctionDef(self, node):
        ...
        dot = Digraph()
        self.process_blocks(node.initial_block, dot)
        dot.render("cfg.gv", view=True)
        
    def process_blocks(self, block, graph):
        ''' TODO: Handle infinite loops '''
        if block.marked:
            return
        if block.start_line_no == "Exit":
            return
        if isinstance(block.statements[0], (ast.If, ast.While)):
            stmt = block.statements[0]
            stmt.body = []
            label = astor.to_source(stmt)
        else:
        ...
        for an_exit in block.exit_blocks:
            next_id = self.process_blocks(an_exit, graph)
            if next_id is not None:
                graph.edge(str(block.id), next_id)
        if block.next_block:
            next_id = self.process_blocks(block.next_block, graph)
            if next_id is not None:
                graph.edge(str(block.id), next_id)
        return str(block.id)
